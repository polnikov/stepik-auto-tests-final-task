...
class ProductPage(BasePage):
    ...
    def should_be_good_message(self):
        assert self.is_element_present(
            *ProductPageLocators.SUCCESS_ADD_TO_BASKET_MESSAGE
        ), "You did not add order to basket"

    def check_order_name_in_basket(self):
        assert self.text_from_element(*ProductPageLocators.ORDER_NAME_ON_PAGE) == \
            self.text_from_element(*ProductPageLocators.ORDER_NAME_IN_MESSAGE),\
            "Order in basket not correct"

    def check_order_price_in_basket(self):
        assert self.text_from_element(*ProductPageLocators.ORDER_PRICE_ON_PAGE) == \
            self.text_from_element(*ProductPageLocators.ORDER_PRICE_IN_BASKET),\
